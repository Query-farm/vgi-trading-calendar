...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-trading-calendar
...
# ATTACH (`${VGI_TRADING_CALENDAR_WORKER}`):
#
#   subprocess : a bare stdio command (`uv run trading_calendar_worker.py`) — the
...
#                        `uv run --python 3.13 <repo>/trading_calendar_worker.py`.
# Optional:
...
WORKER_CMD="${WORKER_CMD:-uv run --python 3.13 $REPO/trading_calendar_worker.py}"
...
# ---------------------------------------------------------------------------
# Per-transport: resolve VGI_TRADING_CALENDAR_WORKER (the LOCATION) and, for the
...
case "$TRANSPORT" in
  subprocess)
    export VGI_TRADING_CALENDAR_WORKER="$WORKER_CMD"
    ;;
  ...
  http)
    ...
    # `--with vgi-python[http]` and the PEP 723 header lists the extra too.
    PORT_FILE="$(mktemp -u "${TMPDIR:-/tmp}/tcal-port.XXXXXX")"
    LOG_FILE="${TMPDIR:-/tmp}/tcal-http-server.log"
    ...
    export VGI_TRADING_CALENDAR_WORKER="http://127.0.0.1:$PORT"
    ;;
  ...
  unix)
    ...
    SOCK="${TMPDIR:-/tmp}/tcal-$$.sock"
    rm -f "$SOCK"
    LOG_FILE="${TMPDIR:-/tmp}/tcal-unix-server.log"
    ...
    export VGI_TRADING_CALENDAR_WORKER="unix://$SOCK"
    ;;
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_TRADING_CALENDAR_WORKER) ..."
