# vgi-trading-calendar — dev and test targets.
#
# Usage:
...
#
# test-sql is self-contained: it points VGI_TRADING_CALENDAR_WORKER at the worker
# run as a uv stdio subprocess (exactly how DuckDB drives it after ATTACH) and
# runs the files under test/sql/. haybarn-unittest is a uv tool:
#   uv tool install haybarn-unittest   # installs ~/.local/bin/haybarn-unittest

# Worker command DuckDB uses for ATTACH (overridable).
WORKER_STDIO    ?= uv run --python 3.13 trading_calendar_worker.py
...
test-sql:
	PATH="$(HAYBARN_BIN):$$PATH" \
		VGI_TRADING_CALENDAR_WORKER="$(WORKER_STDIO)" \
